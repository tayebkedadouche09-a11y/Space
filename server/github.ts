...
export async function createCustomerRepository(sourceUrl: string, sourceBranch: string, customerId: number, productSlug: string): Promise<GitHubRepo> {
  ...
  const target = await githubFetch<{ name: string }>(`/orgs/${ENV.githubOwner}/repos`.replace("/orgs/", ENV.githubOwner.includes("/") ? "/orgs/" : "/user/") === "/user/" ? `/user/repos` : `/orgs/${ENV.githubOwner}/repos`, {
    method: "POST",
    body: JSON.stringify({ name: targetName, private: true, auto_init: false, description: `NUMI customer instance ${productSlug} #${customerId}` }),
  }).catch(async () => {
    return githubFetch<{ name: string }>(`/user/repos`, {
      method: "POST",
      body: JSON.stringify({ name: targetName, private: true, auto_init: false, description: `NUMI customer instance ${productSlug} #${customerId}` }),
    });
  });
  ...
  const tree = await githubFetch<{ tree: GitHubTreeEntry[]; truncated: boolean }>(`/repos/${source.owner}/${source.repo}/git/trees/${ref.object.sha}?recursive=1`);
  if (tree.truncated) throw new Error("Source repository tree is too large for safe automatic cloning. Use the external provisioning worker.");
  const blobs = [];
  for (const entry of tree.tree.filter(e => e.type === "blob")) {
    ...
    const created = await githubFetch<{ sha: string }>(`/repos/${ENV.githubOwner}/${target.name}/git/blobs`, {
      method: "POST",
      body: JSON.stringify({ content: blob.content, encoding: blob.encoding || "base64" }),
    });
    blobs.push({ path: entry.path, mode: entry.mode || "100644", type: "blob", sha: created.sha });
  }
  const newTree = await githubFetch<{ sha: string }>(`/repos/${ENV.githubOwner}/${target.name}/git/trees`, {
    method: "POST",
    body: JSON.stringify({ tree: blobs }),
  });
  const commit = await githubFetch<{ sha: string }>(`/repos/${ENV.githubOwner}/${target.name}/git/commits`, {
    method: "POST",
    body: JSON.stringify({ message: `NUMI provision ${productSlug} for purchase ${customerId}`, tree: newTree.sha }),
  });
  await githubFetch(`/repos/${ENV.githubOwner}/${target.name}/git/refs`, {
    method: "POST",
